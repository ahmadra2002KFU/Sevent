...
} from "../[thread_id]/actions";

const initialState: AdminReplyState = { status: "idle" };
...
export function ReplyComposer(props: ReplyComposerProps) {
  ...
  // Reset the form after a successful send. The shared Textarea component
  // doesn't forward refs, so we drive the reset through the form element.
  useEffect(() => {
  ...
  if (props.disabled) {
    return (
      <div className="rounded-md border border-dashed border-border bg-muted/40 px-3 py-3 text-sm text-muted-foreground">
        {props.disabledReason ?? props.errorClosedLabel}
      </div>
    );
  }
