...
export default async function OrganizerDisputePage({ params }: PageProps) {
  const { id } = await params;
  const locale = (await getLocale()) as SupportedLocale;
  const t = await getTranslations("disputes");
  const { user, admin } = await requireAccess("organizer.bookings");

  // 1. Booking must exist and the viewer must be its organizer. We delegate
  //    that to `resolveOpenDisputeContext` (which also gates the open window).
  ...
  if (openCtx.ok && openCtx.role !== "organizer") {
    redirect(`/supplier/bookings/${id}/dispute`);
  }

  // 2. Load every dispute on this booking (party RLS will return any open by
  //    either side). Empty if no one has filed yet.
  ...
  // 3. Pull all evidence in one batch + author profile names.
  ...
  // 4. Has the viewer themselves opened a dispute (any status)?
  ...
  // Bind bookingId / disputeId into the server actions for the client forms.
  const boundOpenAction = openDisputeAction.bind(null, id);

  return (
    <section className="flex flex-col gap-6">
      <Button asChild variant="ghost" size="sm" className="w-fit">
        <Link href={`/organizer/bookings/${id}`}>
      ...
      {/* Existing disputes (newest first). Viewer can add evidence to any. */}
      {disputes.length > 0 ? (
