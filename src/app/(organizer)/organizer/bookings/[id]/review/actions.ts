...
export async function submitReviewAction(
  ...
  // Notify the reviewee — they know a review came in, but the rating stays
  // suppressed until both parties submit or the window closes (the publish
  // cron handles publication).
  try {
    await createNotification({
      supabase: admin,
      user_id: ctx.reviewee_profile_id,
      kind: "review.submitted",
      payload: { booking_id: ctx.booking.id, from_role: "organizer" },
    });
  } catch (e) {
    console.error("[submitReviewAction organizer] notify failed", e);
  }

  revalidatePath(`/organizer/bookings/${ctx.booking.id}`);
  revalidatePath(`/organizer/bookings/${ctx.booking.id}/review`);
